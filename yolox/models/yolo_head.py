...
class YOLOXHead(nn.Module):
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            # x0 = x.flatten(2).transpose(1, 2).clone()  # B w*h C
            # Reasoning_layer = Transformer(input_dim)
            # x1 = Reasoning_layer(x0)

            # x = torch.cat((x0, x1), dim = 2) # [1, 64, 256]
            # x = x.transpose(1, 2) # B C w*h
            # x = x.reshape(batch, input_dim*2, w, h) # B C w h

            # mlp = nn.Conv2d(input_dim*2, input_dim, kernel_size=1, stride=1)
            ...
            cls_feat = cls_conv(cls_x) # 分类解耦分支中的两个3 * 3的卷积
            ...
            # sns.set()
            # fig = plt.figure()
            # sns_plot = sns.heatmap(cls_output[0,0,...].detach().cpu().numpy(), cmap="rainbow")
            # # fig.savefig("heatmap.pdf", bbox_inches='tight') # 减少边缘空白
            # plt.show()

            reg_feat = reg_conv(reg_x) # 回归解耦分支中的两个3 * 3的卷积
    ...
    def get_assignments(
        ...
        # 初步筛选：根据anchor中心点、gt_box中心点初步筛选正样本锚框
    ...
    def get_in_boxes_info(
        ...
        center_radius = 1.5 # single positive
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        ...
        # 4. 根据matching_matrix生成fg_mask相关信息
