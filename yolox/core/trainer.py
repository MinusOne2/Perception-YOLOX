...
class Trainer:
    def __init__(self, exp, args):
        ...
        self.amp_training = args.fp16 # Adopting mix precision training
    ...
    def train_one_iter(self):
        ...
        # img = inps[0, ...].permute(1, 2, 0) / 255
        # plt.imshow(img.detach().cpu().numpy())
        # plt.axis("off")
        # plt.show()


        inps = inps.to(self.data_type) # [5, 3, 640, 640]
        targets = targets.to(self.data_type) # [5, 120, 5]
    ...
    def before_train(self):
        ...
        self.prefetcher = DataPrefetcher(self.train_loader) # speedup your pytorch dataloader
    ...
    def before_epoch(self):
        ...
        if self.epoch + 1 == self.max_epoch - self.exp.no_aug_epochs or self.no_aug: # 后面15个epochs不使用mosaic数据增强，增加L1 loss
    ...
    def resume_train(self, model):
        if self.args.resume:
            ...
            ckpt = torch.load(ckpt_file, map_location=self.device) # 这里包含了保存的最后一次的epochs
    ...
    def evaluate_and_save_model(self):
        ...
        with adjust_status(evalmodel, training=False):
            ap50_95, ap50, ap70, summary = self.exp.eval(
